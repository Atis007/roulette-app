...
import {
  ...
  Animated, Alert, KeyboardAvoidingView, Platform, StyleSheet,
...
function PlayerItem({ name, onRemove, onEdit }: { name: string; onRemove: () => void; onEdit: (newName: string) => void }) {
  ...
  return (
    <Animated.View style={[styles.playerItem, { opacity, transform: [{ translateX }] }]}>
      {editing ? (
        <TextInput
          value={draft}
        ...
        <TouchableOpacity style={{ flex: 1, marginRight: scale(10) }} onPress={() => { setDraft(name); setEditing(true); }} activeOpacity={0.7}>
...
export function PlayerInputScreen() {
  ...
  const { players, addPlayer, removePlayer, editPlayer, language, loadQuestions, questionsLoading } = useGame();
        ...
        <View style={styles.container}>
          <View style={styles.inputCard}>
            ...
            <TextInput
              ...
              autoCapitalize="characters"
            />
            <BouncyButton color="blue" size="md" style={{ width: '100%' }} onPress={handleAdd}>
              + {t.addPlayer}
            </BouncyButton>
          </View>
          ...
          <View style={styles.listCard}>
            ...
            >
              {players.length === 0 ? (
                ...
                players.map((p, i) => (
                  <PlayerItem key={p} name={p} onRemove={() => removePlayer(i)} onEdit={(newName) => editPlayer(i, newName)} />
                ))
              )}
